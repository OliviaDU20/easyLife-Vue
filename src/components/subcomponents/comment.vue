	...
	export default{
		...
		methods:{
			getComments(){ //获取评论
				this.$http.get("http://www.liulongbin.top:3005/api/getcomments/"+this.id+"?pageindex="+this.pageIndex).then(result=>{
					if (result.body.status === 0) {
						...
						//每当获取评论的时候，不要把老数据清空覆盖，而是应该以老数据，并拼接上新数据
			...
			postComment(){//发表评论
				...
				this.$http.post('http://www.liulongbin.top:3005/api/postcomment/'+this.$route.params.id,{content:this.msg.trim()}).then(function(result){
						if (result.body.status === 0) {
							//拼接出一个评论对象
